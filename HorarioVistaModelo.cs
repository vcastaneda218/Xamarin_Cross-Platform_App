...
{
    public class HorarioVistaModelo : INotifyPropertyChanged
    {
        ...
        public HorarioVistaModelo(int idProfesor, int idDia)
        {
            ElHorario = new ObservableCollection<Horario>();

            GetHorarioCommand = new Command(
                async () => await GetHorario(idProfesor, idDia),
                () => !IsBusy
                );
        }

        async Task GetHorario(int idProfesor, int idDia)
        {
            if (!IsBusy)
            {
                ...
                try
                {
                    ...
                    var Items = await Repository.GetHorario(idProfesor, idDia);
                    ElHorario.Clear();
